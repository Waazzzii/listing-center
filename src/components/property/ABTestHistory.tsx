...
const STATUS_BADGES: Record<string, { bg: string; text: string }> = {
  ...
  active: { bg: 'bg-secondary', text: 'text-secondary-foreground' },
  snapshot_due: { bg: 'bg-health-orange/15', text: 'text-health-orange' },
...
const RESULT_BADGES: Record<string, { bg: string; text: string; label: string }> = {
  positive: { bg: 'bg-health-green/15', text: 'text-health-green', label: 'Positive' },
  negative: { bg: 'bg-destructive/15', text: 'text-destructive', label: 'Negative' },
  no_change: { bg: 'bg-muted', text: 'text-muted-foreground', label: 'No Change' },
};
...
export default function ABTestHistory({ tests, isLoading }: ABTestHistoryProps) {
  if (isLoading) {
    return (
      <div className="bg-card rounded-lg border border-border p-6">
        <h3 className="text-sm font-semibold text-muted-foreground mb-4">A/B Test History</h3>
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6">
      ...
      {tests.length === 0 ? (
        <p className="text-sm text-muted-foreground">No A/B tests for this property yet.</p>
      ) : (
        <div className="space-y-3">
          ...
          {active.map((test) => {
            ...
            return (
              <div key={test.id} className="border border-chart-2/30 rounded-lg p-3 bg-chart-2/5">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-foreground">{test.test_type}</span>
                  <span className={`text-xs px-1.5 py-0.5 rounded ${statusStyle.bg} ${statusStyle.text}`}>
                    {test.status.replace('_', ' ')}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{test.thesis}</p>
          ...
          {completed.map((test) => {
            ...
            return (
              <div key={test.id} className="border border-border rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-muted-foreground">{test.test_type}</span>
                ...
                <p className="text-xs text-muted-foreground mt-1">{test.thesis}</p>
                {test.result_summary && (
                  <p className="text-xs text-muted-foreground mt-1 italic">{test.result_summary}</p>
                )}
